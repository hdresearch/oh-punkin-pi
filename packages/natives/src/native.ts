...
import { $env, getNativesDir, logger } from "@ohp/utils";
...
const userDataDir =
	process.platform === "win32"
		? path.join(Bun.env.LOCALAPPDATA || path.join(os.homedir(), "AppData", "Local"), "ohp")
...
function loadNative(): NativeBindings {
	...
	if (isCompiledBinary) {
		...
		helpMessage =
			"If installed via npm/bun, try reinstalling: bun install @ohp/natives\n" +
