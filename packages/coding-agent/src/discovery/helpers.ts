...
import { FileType, glob } from "@ohp/natives";
import { CONFIG_DIR_NAME, getConfigDirName, getProjectDir, tryParseJson } from "@ohp/utils";
 ...
 *
 * Walk order:
 * 1. Walk up from `cwd` looking for the nearest directory containing `.ohp/`.
 *    The first match returns `<dir>/.ohp/plugins/installed_plugins.json`.
 * 2. If no `.ohp/` is found, rescan from `cwd` upward looking for `.git`.
 *    The git root is used as an anchor: `<gitRoot>/.ohp/plugins/installed_plugins.json`.
...
export async function resolveActiveProjectRegistryPath(cwd: string): Promise<string | null> {
	// Pass 1: walk up looking for an existing .ohp/ directory (nearest wins).
	// Stop before os.homedir() — ~/.ohp/ is the user-level config dir, not a project root.
...
/**
 * Like resolveActiveProjectRegistryPath, but falls back to `<cwd>/.ohp/plugins/installed_plugins.json`
 * when no project anchor (.ohp/ or .git/) is found.
 *
 * Use this when the caller accepts an explicit --scope project so that installing into a freshly
 * bootstrapped directory (no .ohp/ or .git/ yet) works: writeInstalledPluginsRegistry auto-creates
 ...
 * Reads ~/.claude/plugins/installed_plugins.json and ~/.ohp/plugins/installed_plugins.json,
...
export async function listClaudePluginRoots(
	...
	// ── OHP installed plugins registry ───────────────────────────────────────
	// OHP registry is authoritative: its entries replace Claude's entries for the same plugin ID.
	...
	if (ompContent) {
		...
		if (ompRegistry) {
			for (const [pluginId, entries] of Object.entries(ompRegistry.plugins)) {
				...
				// OHP is authoritative: drop all Claude-sourced entries for this plugin ID
			...
			warnings.push(`Failed to parse OHP plugin registry: ${ompRegistryPath}`);
		}
	}

	// ── Project-scoped OHP registry ────────────────────────────────────────
	// Loaded from the nearest .ohp/plugins/installed_plugins.json relative to cwd.
 ...
 * These are prepended to the cache with highest precedence (before OHP/Claude entries).
