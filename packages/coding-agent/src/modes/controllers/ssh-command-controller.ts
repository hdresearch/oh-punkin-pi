 ...
 */
import { Spacer, Text } from "@ohp/tui";
import { getProjectDir, getSSHConfigPath } from "@ohp/utils";
	...
	async #handleList(): Promise<void> {
		try {
			...
			if (userHosts.length > 0) {
				lines.push(theme.fg("accent", "User level") + theme.fg("muted", ` (~/.ohp/agent/ssh.json):`));
			...
			if (projectHosts.length > 0) {
				lines.push(theme.fg("accent", "Project level") + theme.fg("muted", ` (.ohp/ssh.json):`));
