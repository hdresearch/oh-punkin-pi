 ...
 * .bashrc/.zshrc/config.fish, which can be sourced before each command to provide a familiar
 * shell experience. For non-bash/zsh shells (e.g. fish), the snapshot extracts exported
 * environment variables (especially PATH) and emits them as POSIX export statements.
 */
...
function generateSnapshotScript(shell: string, snapshotPath: string, rcFile: string): string {
	if (shell.includes("fish")) {
		return generateFishSnapshotScript(snapshotPath);
	}
	return generateBashZshSnapshotScript(shell, snapshotPath, rcFile);
}

/**
 * Generate a fish-native script that extracts the environment fish would have
 * after sourcing config.fish, then emits POSIX export statements for Brush.
 *
 * We invoke fish in login mode (-l) so that config.fish and conf.d/ are sourced.
 * The output is a POSIX-compatible snapshot that Brush can source.
 */
function generateFishSnapshotScript(snapshotPath: string): string {
	const escapedPath = snapshotPath.replace(/'/g, "'\\'");
	// Fish script: emit each exported variable as a POSIX export statement.
	// fish -l sources config.fish + conf.d/*.fish, giving us the full user env.
	// We extract all exported variables and write them as 'export KEY="VALUE"' lines.
	// PATH elements are joined with ':' (fish stores them as a list internally).
	return [
		`set -l snapshot_file '${escapedPath}'`,
		`echo '# Shell snapshot - generated by ohp agent (fish)' > $snapshot_file`,
		`# Export all user-exported variables as POSIX statements`,
		`for name in (set --export --names)`,
		`    # Skip internal fish variables and common non-portable ones`,
		`    switch $name`,
		`        case 'FISH_*' 'fish_*' '_*' 'SHLVL' 'PWD' 'OLDPWD' 'HOME' 'USER' 'LOGNAME' 'HOSTNAME'`,
		`            continue`,
		`    end`,
		`    set -l val (string join ':' $$name)`,
		`    # Escape single quotes in the value`,
		`    set -l escaped (string replace --all "'" "'\\''" -- $val)`,
		`    echo "export $name='$escaped'" >> $snapshot_file`,
		`end`,
		`# Emit fish aliases as POSIX alias statements`,
		`for name in (alias | string match -r '^alias (\\S+)' | string match -r '\\S+' | head -n 500)`,
		`    # Fish alias output format varies; extract name and definition via 'functions'`,
		`end`,
	].join("\n");
}

function generateBashZshSnapshotScript(shell: string, snapshotPath: string, rcFile: string): string {
...
export async function getOrCreateSnapshot(
	...
	try {
		...
		// For fish: spawn with fish so it interprets the fish-syntax snapshot script.
		// For bash/zsh: spawn with the user's shell (which understands the script).
