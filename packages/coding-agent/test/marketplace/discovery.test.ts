 ...
 * @ohp/natives (native Rust addon via glob). We cannot call it here.
 *
 * Instead these tests validate the structural contract that listClaudePluginRoots
 * depends on:
 *   1. OMP registry lives at path.join(home, ".ohp", "plugins", "installed_plugins.json")
 *      (matches getConfigDirName() == ".ohp")
...
// Kept here to avoid importing helpers.ts (which pulls in @ohp/natives).
...
// Matches getConfigDirName() — single source of truth is in @ohp/utils,
// but we know the value is ".ohp" and hardcoding it here keeps tests free of
// native-addon transitive imports.
const OMP_CONFIG_DIR = ".ohp";
...
let tmpHome: string;
/** ~/.ohp/plugins/installed_plugins.json inside tmpHome */
...
describe("OMP registry path contract", () => {
	it("OMP registry lives at home/.ohp/plugins/installed_plugins.json", () => {
		// This is the path that listClaudePluginRoots reads.
		// Any change to this path must be reflected in helpers.ts.
		const expected = path.join(tmpHome, ".ohp", "plugins", "installed_plugins.json");
		expect(ompRegistryPath).toBe(expected);
	});

	it("OMP config dir name is .ohp", () => {
		...
		expect(OMP_CONFIG_DIR).toBe(".ohp");
	});
});
