 ...
 *
 * Note: helpers.ts imports @ohp/natives (Rust addon via glob).
...
import type { InstalledPluginEntry } from "@ohp/coding-agent/extensibility/plugins/marketplace";
import {
	addInstalledPlugin,
	buildPluginId,
	readInstalledPluginsRegistry,
	writeInstalledPluginsRegistry,
} from "@ohp/coding-agent/extensibility/plugins/marketplace";
import {
...
describe("resolveActiveProjectRegistryPath", () => {
	...
	it("walk-up finds nearest .ohp/ directory", async () => {
		// Layout: tmpDir/.ohp/   +   tmpDir/sub/nested/  (cwd)
		// Resolver must climb from cwd → sub → tmpDir and find .ohp/ there.
		fs.mkdirSync(path.join(tmpDir, ".ohp"), { recursive: true });
		const cwd = path.join(tmpDir, "sub", "nested");
		fs.mkdirSync(cwd, { recursive: true });

		const result = await resolveActiveProjectRegistryPath(cwd);

		expect(result).toBe(path.join(tmpDir, ".ohp", "plugins", "installed_plugins.json"));
	});

	it("walk-up stops at the nearest .ohp/ — does not skip to a more distant one", async () => {
		// Layout: tmpDir/.ohp/   +   tmpDir/sub/.ohp/   +   tmpDir/sub/nested/  (cwd)
		// Resolver must stop at tmpDir/sub/.ohp/, not climb further to tmpDir/.ohp/.
		fs.mkdirSync(path.join(tmpDir, ".ohp"), { recursive: true });
		fs.mkdirSync(path.join(tmpDir, "sub", ".ohp"), { recursive: true });
		const cwd = path.join(tmpDir, "sub", "nested");
		fs.mkdirSync(cwd, { recursive: true });

		const result = await resolveActiveProjectRegistryPath(cwd);

		expect(result).toBe(path.join(tmpDir, "sub", ".ohp", "plugins", "installed_plugins.json"));
	});

	it("falls back to .git root when no .ohp/ exists", async () => {
		// Layout: tmpDir/.git/   +   tmpDir/sub/  (cwd)
		// No .ohp/ anywhere → second pass finds .git/ at tmpDir.
	...
	it("returns null when neither .ohp/ nor .git/ found anywhere in the tree", async () => {
		// Start at the filesystem root — guaranteed to have no .ohp/ or .git/ ancestors.
	...
	it("does not treat ~/.git as a project root (pass-2 home-dir guard)", async () => {
		...
		// resolveActiveProjectRegistryPath must NOT return ~/.ohp/.../installed_plugins.json.
		...
		try {
			// Start from a tmpDir that has no .ohp/ or .git/ of its own.
			const result = await resolveActiveProjectRegistryPath(tmpDir);
			// Must not resolve to the home-dir OMP registry.
			const homeOmpPath = path.join(homeDir, ".ohp", "plugins", "installed_plugins.json");
...
describe("listClaudePluginRoots — project shadows user", () => {
	...
	beforeEach(() => {
		...
		// Create .ohp/ in project so resolveActiveProjectRegistryPath finds it.
		fs.mkdirSync(path.join(tmpProject, ".ohp", "plugins"), { recursive: true });

		userRegPath = path.join(tmpHome, ".ohp", "plugins", "installed_plugins.json");
		fs.mkdirSync(path.dirname(userRegPath), { recursive: true });

		projectRegPath = path.join(tmpProject, ".ohp", "plugins", "installed_plugins.json");
	});
