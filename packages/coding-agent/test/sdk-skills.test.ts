...
import type { Skill } from "@ohp/coding-agent/sdk";
...
describe("createAgentSession skills option", () => {
	...
	beforeEach(() => {
		...
		// Create skill in .ohp/skills/ for native project-level discovery
		skillsDir = path.join(tempDir, ".ohp", "skills", "test-skill");
		fs.mkdirSync(skillsDir, { recursive: true });
		originalHome = process.env.HOME;
		tempHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-sdk-home-"));
		process.env.HOME = tempHomeDir;
		const nativeUserSkillsDir = path.join(tempHomeDir, ".ohp", "agent", "skills");
...
`,
	...
	it("should still discover project skills when user skills directory is missing", async () => {
		const userAgentDir = path.join(tempHomeDir, ".ohp", "agent");
