...
import { createAgentSession } from "@ohp/coding-agent/sdk";
import { SessionManager } from "@ohp/coding-agent/session/session-manager";
import { buildSystemPrompt, loadProjectContextFiles, loadSystemPromptFiles } from "@ohp/coding-agent/system-prompt";
...
describe("SYSTEM.md prompt assembly", () => {
	...
	it("renders SYSTEM.md exactly once when it is used as the custom base prompt", async () => {
		...
		const systemDir = path.join(projectDir, ".ohp");
	...
	it("prefers project SYSTEM.md over user SYSTEM.md", async () => {
		const projectDir = path.join(tempDir, "project");
		fs.mkdirSync(path.join(projectDir, ".ohp"), { recursive: true });
		fs.mkdirSync(path.join(tempHomeDir, ".ohp", "agent"), { recursive: true });
		fs.writeFileSync(path.join(tempHomeDir, ".ohp", "agent", "SYSTEM.md"), "User SYSTEM prompt");
		fs.writeFileSync(path.join(projectDir, ".ohp", "SYSTEM.md"), "Project SYSTEM prompt");
