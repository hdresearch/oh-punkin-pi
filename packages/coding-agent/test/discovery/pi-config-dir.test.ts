...
import type { LoadContext } from "@ohp/coding-agent/capability/types";
import { getConfigDirs } from "@ohp/coding-agent/config";
import { getUserPath } from "@ohp/coding-agent/discovery/helpers";
...
describe("PI_CONFIG_DIR", () => {
	...
	test("getConfigDirs respects PI_CONFIG_DIR for user base", () => {
		...
		expect(result[0]).toEqual({ path: expected, source: ".ohp", level: "user" });
	});
});
