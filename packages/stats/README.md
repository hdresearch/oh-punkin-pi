...
- **Session log parsing**: Reads JSONL session logs from `~/.ohp/agent/sessions/`
...
```typescript
import { getDashboardStats, syncAllSessions } from "@ohp/stats";
...
- **Session logs**: `~/.ohp/agent/sessions/` (JSONL files)
- **Stats database**: `~/.ohp/stats.db` (SQLite)
