...
- Updated environment variable access to use `getEnv()` utility function from `@ohp/utils` for consistent handling
...
- Moved `wrapTextWithAnsi` export to `@ohp/natives` package
...
- Removed `truncateToWidth`, `sliceWithWidth`, and `extractSegments` functions from public API (now re-exported directly from @ohp/natives)
