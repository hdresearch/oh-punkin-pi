...
- Examples (adjust to match the actual packages you are porting):
  - `@mariozechner/pi-coding-agent` → `@ohp/coding-agent`
  - `@mariozechner/pi-agent-core` → `@ohp/agent-core`
  - `@mariozechner/pi-tui` → `@ohp/tui`
  - `@mariozechner/pi-ai` → `@ohp/ai`
...
- In coding-agent, never use `console.log`/`console.warn`/`console.error`; use `logger` from `@ohp/utils`.
...
| Upstream                                           | Our Fork                                | Reason                                  |
| -------------------------------------------------- | --------------------------------------- | --------------------------------------- |
| `clipboard.ts` + `clipboard-image.ts` (tool files) | `@ohp/natives` clipboard module | Merged into N-API native implementation |
...
- `clipboard-image.ts` — clipboard is in `@ohp/natives` N-API module
