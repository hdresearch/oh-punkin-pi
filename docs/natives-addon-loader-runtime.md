...
This document deep-dives the addon loading/validation layer in `@ohp/natives`: how `native.ts` decides which `.node` file to load, when embedded payload extraction runs, and how startup failures are reported.
...
- reinstall hint (`bun install @ohp/natives`),
