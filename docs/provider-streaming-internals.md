...
This document explains how token/tool streaming is normalized in `@ohp/ai`, then propagated through `@ohp/agent-core` and `coding-agent` session events.
...
## Unified stream contract in `@ohp/ai`
